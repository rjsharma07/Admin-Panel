...
export default function Sidebar({ 
  ...
  return (
    <aside 
      className={`bg-[#0f172a] fixed inset-y-0 left-0 z-[60] flex flex-col border-r border-slate-800/50 transition-all duration-300 ease-in-out h-screen ${
        isCollapsed 
      ...
      <div className="flex flex-col h-full justify-between pb-6">
        <div>
          ...
          <div className="h-20 w-full flex items-center px-4 relative shrink-0">
            <div className={`flex items-center w-full transition-all duration-300 ${isCollapsed ? "lg:justify-center" : "justify-start pl-2 gap-3"}`}>
              ...
              {/* Brand Text */}
              <span className={`text-[15px] font-black text-white tracking-widest whitespace-nowrap transition-all duration-300 min-w-0 ${
                isCollapsed ? "opacity-0 absolute scale-75 pointer-events-none" : "opacity-100 relative scale-100"
              }`}>
                ADMIN PANEL
              </span>
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link 
                  ...
                  className={`flex items-center px-4 py-3 text-sm font-bold transition-all duration-200 group relative rounded-xl ${
                ...
                >
                  ...
                  <span className={`relative z-10 whitespace-nowrap overflow-hidden transition-all duration-300 ${
                    isCollapsed ? "opacity-0 absolute scale-95" : "opacity-100 relative scale-100"
                  }`}>
                    {item.name}
                  </span>
                </Link>
        ...
        <div className={`px-4 mt-auto border-t border-slate-800/50 pt-6 space-y-4 transition-all duration-300 ${isCollapsed ? "lg:items-center" : ""}`}>
          ...
          <button 
            ...
            className={`w-full flex items-center py-3 text-sm font-bold transition-all group rounded-xl ${
              isCollapsed ? "lg:justify-center" : "px-4 gap-3 text-slate-400 hover:text-rose-400 hover:bg-rose-600/5"
            }`}
          >
            <div className={`transition-opacity duration-200 ${
              isCollapsed ? "lg:text-slate-400 lg:group-hover:text-rose-400" : "opacity-50 group-hover:opacity-100"
            }`}>
              <svg 
                className="w-5 h-5" 
                fill="none" 
